# -*- coding: utf-8 -*-
"""
Created on Thu Sep 19 05:28:47 2019
...
class Model():
    ...
    def lossFn(self, Y_train, Y_predict):
        ...
        n = Y_train.shape[0]        
        loss = (1/(2*n))*(np.dot(np.transpose(Y_diff), Y_diff)) + (config.LAMBDA/(2*n))*(np.dot(self.W.T, self.W) - (self.W[0]*self.W[0]))
    ...
    def plot_best_fit_line(self, data):
        ...
        plt.xlabel('X')
        plt.ylabel('Y')
        plt.title('L2 Regularization Using Gradient Descent')
        plt.savefig(config.OUT_DIR + 'BestFitLinePlot_L2Regularization_GradDescent')
        plt.show()