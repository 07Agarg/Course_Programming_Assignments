# -*- coding: utf-8 -*-
"""
Created on Sun Sep  8 17:57:45 2019

@author: ashima
"""

#Reference: https://www.aclweb.org/anthology/P09-1054
...
class Model():
    ...
    def backward(self, X, Y, p):
        t = p - Y
        n = Y.shape[0]
        gradient = (np.dot(np.transpose(X), t))/n + (config.LAMBDA/(2*n))*(np.sign(self.W))
        gradient[0] = gradient[0] - (config.LAMBDA/(2*n))*(np.sign(self.W[0]))
        self.W = self.W - (config.LEARNING_RATE)*(gradient)
    
    def lossFn(self, Y, p):
        n = Y.shape[0]
        loss = (-Y * np.log(p) - (1 - Y) * np.log(1 - p)).mean() + (config.LAMBDA/(2*n))*(np.sum(np.abs(self.W)) - np.abs(self.W[0]))
    ...
    def train(self, data):
        ...
        #Store L1 Weights
        with open(os.path.join(config.OUT_DIR, config.L1_WEIGHTS) , "w") as file:
    ...
    def calculate_accuracy(self, X, Y, string):
        ...
        if string == "None":
            ...
            print("{} Accuracy {} using L1 Regularization".format(string, (accuracy/Y.shape[0])*100))
